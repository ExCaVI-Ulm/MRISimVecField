function [M_prop] = ExcMatrixCalc_block(settings, B_t_woRF, indices_start, indices_end, B1TX_spatial, blocks, j, Minit, reco_flag)
%ExcMatrixCalc_block Matrix based propagation for an RF block
...
%           -M_prop: propagated Magnetization vector
    ...
    M_exc = zeros(matrixsize_signal,3);
    w = settings.TX.w;
    for ll = 1:matrixsize_signal
        %transformation into tilted B0-frame
        u_B(ll,:) = B_t_woRF(1,ll,:) / norm(squeeze(B_t_woRF(1,ll,:)));    %calculate unit vector

        %rot axis
        rot_axis(ll,:) = cross(squeeze(B_t_woRF(1,ll,:)), [0;0;1]);
        if norm(rot_axis(ll,:)) ~= 0
            rot_axis(ll,:) = rot_axis(ll,:) / norm(cross(squeeze(B_t_woRF(1,ll,:)), [0;0;1]));
        end
        theta = acos(squeeze(B_t_woRF(1,ll,3)) / norm(squeeze(B_t_woRF(1,ll,:))));
        ...
        u_B1_trafo(ll,3) = 0;%
        %excitation
        a_rot_exc = [cos(blocks{j}.initPhase);sin(blocks{j}.initPhase);0] + (norm(squeeze(B_t_woRF(1,ll,:)))*settings.general.gamma - w(1))/settings.general.gamma * [0;0;1];
        ...
        M_exc(ll,:) = R_CoSy_Mtheta*R_rotframe*R_exc_alpha*R_rotframeB*R_CoSy_theta*Minit(ll,:).';
    end

    M_prop = permute(repmat(M_exc, [1 1 (indices_end(j)-indices_start(j)+1)]), [3 1 2]);
end